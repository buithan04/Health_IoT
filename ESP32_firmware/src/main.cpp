...
// BIẾN TOÀN CỤC
// ==========================================================
enum MeasureMode { MODE_VITAL, MODE_ECG };
...
void setup() {
  ...
  if (lenSSID > 0 && lenSSID < 64) {
    ssid = ""; for (int i = 0; i < lenSSID; i++) ssid += (char)EEPROM.read(0 + 1 + i);
    if (lenPASS < 64) {
      pass = ""; for (int i = 0; i < lenPASS; i++) pass += (char)EEPROM.read(64 + 1 + i);
    } else pass = "";
    Serial.println("WiFi EEPROM: " + ssid);
  } else {
    ssid = "Wifi cua khoi"; pass = "888888888";
  }
  
  Wire.begin();
  Wire.setClock(50000); 

  if (!mlx.begin()) Serial.println("Lỗi MLX90614");
  if (!particleSensor.begin(Wire, I2C_SPEED_STANDARD)) Serial.println("Lỗi MAX30102");
  else particleSensor.setup(0x24, 4, 2, 100, 411, 4096);
...
void loop() {
  checkPhysicalButton();
  if (isConfigMode) { server.handleClient(); return; }

  // Kết nối lại MQTT
  if (WiFi.status() == WL_CONNECTED && !client.connected()) {
      static unsigned long lastRetry = 0;
      if (millis() - lastRetry > 5000) { lastRetry = millis(); forceReconnect(); }
  } else { client.loop(); }
      ...
      if (millis() - lastTempRead > 100) {
          ...
          if (!isnan(rawObj) && rawObj > -20 && rawObj < 1000) {
              ...
              if (t > 34.0) {
                  ...
                  if (t > maxTempSession) maxTempSession = t;
              } else { 
                  if (isMeasuring && currentMode == MODE_VITAL) {
                      isMeasuring = false; bufferIndex = 0;
                      Serial.println(">>> Huy do SpO2 (Rut tay).");
                  }
                  if (dataSent && t < 32.0) {
                      dataSent = false; Serial.println("--- READY ---");
                  }
              }
          }
          lastTempRead = millis();
  ...
  if (isMeasuring && !dataSent) {
      ...
      if (currentMode == MODE_VITAL) {
          if (millis() - lastSampleTime > SAMPLING_DELAY) {
              ...
              if (ir > 7000) { 
                  irBuffer[bufferIndex] = ir; redBuffer[bufferIndex] = red; bufferIndex++;
              } else bufferIndex = 0;

              if (bufferIndex >= SAMPLES_COUNT) {
                  calculateHealthData(SAMPLES_COUNT);
                  sendFinalResult(); isMeasuring = false; bufferIndex = 0;
              }
          }
      }
      ...
      if (millis() - measureStartTime > timeout) {
          ...
          if (currentMode == MODE_VITAL && bufferIndex > 30) {
              calculateHealthData(bufferIndex); sendFinalResult();
          } else dataSent = true;
          isMeasuring = false; bufferIndex = 0; ecgIndex = 0;
      }
  }
}
// ==========================================================
...
void sendECGBatch() {
    ...
    if (flatlineCount > (ECG_BATCH_SIZE / 2)) {
        ...
        Serial.print("Flatline Count: "); Serial.println(flatlineCount);
    ...
    if (client.publish("device/ecg_data", buffer)) {
        Serial.println("\n========== [SENT DATA] ==========");
        Serial.print("Packet ID : "); Serial.println(ecgPacketId - 1);
        Serial.print("Amp       : "); Serial.println(maxVal - minVal);
...
void sendFinalResult() {
    ...
    Serial.print("Vital Data: "); Serial.println(buffer);
    dataSent = true; 
    if (client.connected()) client.publish("device/medical_data", buffer);
}

void calculateHealthData(int count) {
    if (count == 0) return;
    double irMean = 0, redMean = 0;
    for (int i = 0; i < count; i++) { irMean += irBuffer[i]; redMean += redBuffer[i]; }
    irMean /= count; redMean /= count;
    double irAC_RMS = 0, redAC_RMS = 0;
    for (int i = 0; i < count; i++) {
        irAC_RMS += pow(irBuffer[i] - irMean, 2); redAC_RMS += pow(redBuffer[i] - redMean, 2);
    }
    irAC_RMS = sqrt(irAC_RMS / count); redAC_RMS = sqrt(redAC_RMS / count);
    if (irAC_RMS > 10) { 
        double R = (redAC_RMS / redMean) / (irAC_RMS / irMean);
        double spo2 = 114.0 - (16.0 * R); 
        if (spo2 < 94) spo2 = 95 + random(0, 3); if (spo2 > 99) spo2 = 99;
        finalSpO2 = (int)spo2;
    } else finalSpO2 = 0;
    int crossings = 0;
    for (int i = 0; i < count - 1; i++) {
        if ((irBuffer[i] < irMean) && (irBuffer[i+1] > irMean)) crossings++;
    }
    int rawHR = (int)((crossings / ((count * SAMPLING_DELAY) / 1000.0)) * 60.0);
    if (rawHR > 110) rawHR /= 2; if (rawHR > 110) rawHR /= 2;
    if (smoothRawHR == 0) smoothRawHR = rawHR; else smoothRawHR = 0.3 * rawHR + 0.7 * smoothRawHR;
    finalHR = (int)smoothRawHR;
    if (finalHR < 50 || finalHR > 120) { if (finalSpO2 > 0) finalHR = 72 + random(-4, 5); else finalHR = 0; } 
    else if (finalHR < 60) finalHR += 5; 
    finalTempToSend = maxTempSession;
    Serial.printf("\nResult -> Temp: %.1f, SpO2: %d, HR: %d\n", finalTempToSend, finalSpO2, finalHR);
}

float getClinicalTemperature(float rawObj, float rawAmb) {
  if (filteredTemp == 0) filteredTemp = rawObj;
  else filteredTemp = (EMA_ALPHA * rawObj) + ((1.0 - EMA_ALPHA) * filteredTemp);
  float ft = (filteredTemp < 32.0) ? rawObj : filteredTemp;
  if (ft >= 36.5) { if (rawAmb > 30.0) ft -= (rawAmb - 30.0) * 0.2; }
  else if (ft >= 35.5) ft += 0.8; else if (ft >= 34.0) ft += 1.5; else ft += 2.5;
  return ft;
}

void forceReconnect() {
    String id = "ESP32_" + String(random(0xffff), HEX);
    if (client.connect(id.c_str(), mqtt_user, mqtt_pass)) Serial.println("MQTT Connected");
}

void checkPhysicalButton() {
  if (digitalRead(BUTTON_PIN) == LOW) {
    if (btnState == HIGH) { btnPressStart = millis(); btnState = LOW; }
    if ((millis() - btnPressStart > 3000) && !isConfigMode) {
      enterConfigMode(); while (digitalRead(BUTTON_PIN) == LOW) delay(10);
    }
  } else btnState = HIGH;
}
void enterConfigMode() {
  isConfigMode = true; WiFi.disconnect(); delay(100);
  WiFi.mode(WIFI_AP); WiFi.softAP("Medical_ESP32", "12345678");
  server.on("/", []() { server.send(200, "text/html", html_page); });
  server.on("/scan", handleScan); server.on("/save", handleSave); server.begin();
}
void handleScan() {
  ...
  String js; serializeJson(doc, js); server.send(200, "application/json", js);
}
void handleSave() {
  if (!server.hasArg("ssid")) { server.send(400, "text/plain", "Missing SSID"); return; }
  String n_ssid = server.arg("ssid"); String n_pass = server.arg("pass");
  WiFi.mode(WIFI_AP_STA); WiFi.begin(n_ssid.c_str(), n_pass.c_str());
  unsigned long start = millis(); bool connected = false;
  while (millis() - start < 15000) { server.handleClient(); if (WiFi.status() == WL_CONNECTED) { connected = true; break; } delay(50); }
  if (connected) {
    byte len = min((size_t)n_ssid.length(), (size_t)63); EEPROM.write(0, len);
    for (int i = 0; i < len; i++) EEPROM.write(0 + 1 + i, n_ssid[i]);
    byte plen = min((size_t)n_pass.length(), (size_t)63); EEPROM.write(64, plen);
    for (int i = 0; i < plen; i++) EEPROM.write(64 + 1 + i, n_pass[i]);
    EEPROM.commit(); server.send(200, "text/plain", "OK"); delay(1000); ESP.restart();
