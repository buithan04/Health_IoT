...
import 'package:health_iot/service/socket_service.dart'; // Import Socket
...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _deleteAllNotifications() async {
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            const Text('Xóa tất cả thông báo'),
          ],
        ),
        content: Text(
          'Xóa VĨNH VIỄN ${_notifications.length} thông báo khỏi hệ thống?\n\nHành động này KHÔNG THỂ hoàn tác!',
          style: GoogleFonts.inter(fontSize: 15, height: 1.5, color: Colors.red[700]),
        ),
        actions: [
          TextButton(
    ...
    // Xóa UI ngay lập tức (Optimistic Update)
    final backupList = List<NotificationModel>.from(_notifications);
    setState(() {
    ...
    if (success) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Đã xóa ${backupList.length} thông báo'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Không thể xóa, vui lòng thử lại'),
          backgroundColor: Colors.red[600],
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
